...
export const ColorFilter = ({ onColorChange }) => {
  ...
  const handleColorChange = selectedColor => {
    if (color.includes(selectedColor)) {
      setColor(prevColor => prevColor.filter(item => item !== selectedColor));
    ...
    <div>
      <div className="w-40 lg:w-60 xl:w-72 px-2.5 py-3 lg:p-3 bg-300 text-200 rounded-lg flex flex-col justify-between align-middle lg:text-base">
        ...
        >
          ...
          {open ? (
            <>
              <ExpandLess onClick={handleOpen} className="cursor-pointer" />
            </>
          ) : (
            <>
              <ExpandMore onClick={handleOpen} className="cursor-pointer" />
            </>
          )}
        </div>
        <div className="text-400">
          <div className={`${open ? "block" : "hidden"} mt-5 text-base`}>
