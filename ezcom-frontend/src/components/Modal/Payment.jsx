...
const CreditCardForm = ({ dataToSend, isPaymentModalOpen, setIsPaymentModalOpen }) => {
        ...
        <div className="modal">
          <div className="flex justify-center bg-400  p-10 rounded-lg ml-20 mt-10  ">
            ...
            <div className="w-96 h-60 text-100 pb-5 pt-10 px-9 mx-1 text-start align-middle bg-blue-500  bg-gradient-to-b from-blue-800 border-400 border-1 rounded-2xl drop-shadow-xl shadow-xl">
              <DeveloperBoardIcon className='text-yellow-400' style={{ fontSize: 52 }} />
              <div className='flex justify-center'>
                <div className="mt-5 text-3xl">{cardNumber.padEnd(16, '•').match(/.{1,4}/g).join(' ')}
                  <div className="mr-10 mt-2 text-sm align-bottom text-end">{expiryDateMonth ? `${expiryDateMonth} / ${expiryDateYear}` : 'MM/YY'}</div>
                  <div className="mt-5 align-bottom text-xs text-start">{cardHolder || 'CARD HOLDER'}</div>
                </div>
              </div>
              {/* <div className="cvv">{cvv.padEnd(3, '•')}</div> */}
            </div>

            <form onSubmit={handleSubmit} className="flex flex-col gap-3 pl-4 justify-center ">
              <div className="">
                <input
                  type="text"
                  name="cardNumber"
              ...
              <div className="flex gap-2 justify-between">
                <button type="submit"
                  className="flex min-w-[68%] justify-center bg-green-500 rounded-xl p-2  text-100 text-xl" onClick={handleSubmit}>
                  PAY NOW
                </button>
                <button className="flex min-w-[30%] justify-center bg-red-500 rounded-xl p-2  text-100 text-xl " onClick={closeModal}>
                  CANCEL
                </button>
